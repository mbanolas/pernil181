...
$ticketsPeriodo=$this->session->ticketsPeriodo ;
$totalNumTickets=sizeof($ticketsPeriodo);
...
//var_dump($this->session->all_userdata());
     ...
     // echo $_SESSION['diferenciaPeriodoImportes'];
echo $botones ?>

<hr>
    <h3>Ticket núm <?php echo $ticketsPeriodo[$posicion] ?><span style="font-weight: lighter">&nbsp;&nbsp;&nbsp;&nbsp; (<?php echo ($posicion+1) ?>/<?php echo $totalNumTickets ?>)</span></h3>
    <?php $alternativa=true ?>

   <?php echo form_open('tickets/recuperarBoka',array('role'=>'form', 'class'=> 'fechas form-horizontal')) ; ?> 
        ...
        <table class="table ticket" >
            <thead>
                <tr id="tituloAlteracion">
                    <th colspan="3" class="col-md-12 izquierda"><?php echo $ticket2['modo'] ?><span id="modificacion" ></span>
                    </th>
                </tr>
...
$(document).ready(function(){
    ...
    $('#guardarModificaciones').click(function(e){
        
        formmodified = 0;
        e.preventDefault();
        $.ajax({
            type: "POST",
            url: "<?php echo base_url() ?>"+"index.php/tickets/prepararBoka2", 
            data: $('form.fechas').serialize(),
            success: function(datos){
               // alert(datos);
                alert("Ticket modificado y guardado.\nProceder con otro Ticket")
                        },
            error: function(){
                alert("Error en el proceso. Informar");
            }
