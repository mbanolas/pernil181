...
$(document).ready(function(){
    ...
    <?php if($tituloRango=="Tickets seleccionados por número") { ?>
        console.log('Num tickets')
        $('#gcrud-search-form > table > thead > tr.filter-row.gc-search-row').addClass('hide')
        $('#gcrud-search-form > table > tfoot > tr:nth-child(2)').addClass('hide')
    <?php } ?>
    ...
    function totalesPie(){
                ...
                if(valor){
                    ...
                    switch(campo){
                            ...
                            case 'Buscar Núm. cliente':
                                where+=" AND t.id_cliente LIKE '%"+valor+"%'"
                            break 
            ...
            $.ajax({
            type:'POST',
            url: "<?php echo base_url() ?>"+"index.php/gestionTablas/ticketsTiendaTotales", 
            data:{sql:sql},
    ...
    $('#gcrud-search-form > div.header-tools > div.floatL.t5 > a').click(function(e){
     ...
     <?php if($tituloRango!="Tickets seleccionados por número") { ?>
    $('<tfoot ><tr><th></th></tr><tr style="border:5px solid blue;"><th style="text-align: left; " colspan="2">Totales selección</th><th id="totalTickets">T</th><th i></th><th i></th><th id="total"></th></tr></tfoot>').insertAfter('tbody')
     <?php } ?>
    ...
    // ver ticket een ventana modal
    $('body').delegate('tr > td:nth-child(1) > div.only-desktops > div > ul > li:nth-child(3) > a','click',function(e)  
    // $('body').delegate('tr td> div> a[href*="tickets"]','click',function(e)  
        {  
            console.log('pulsado ver')
            e.preventDefault()
            var id=$(this).attr('href').substr($(this).attr('href').lastIndexOf("/")+1)
            $( this ).parent().parent().parent().parent().append( " <img class='loader' src='<?php echo base_url() ?>images/ajax-loader-2.gif' height='20' width='20'>" );
          $.ajax({
        type:'POST',
