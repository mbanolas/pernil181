...
<input type="hidden" value="0" id="formmodified">    
<div id="div1">
        </div>
    <div id="div1A">
        </div>
        ...
        <table class="table ticket" >
            <thead>
                ...
                <tr>
                    ...
                    <td class="col-md-6 izquierda ">
                        <?php echo form_dropdown('cliente', $id_clientes, $ticket['cliente'],array('id'=>'id_cliente','class'=>'cambiar')); ?>
                        <!--
                        <select name="cliente" id="id_cliente" class="cambiar prueba">
                            <?php  foreach($id_clientes as $k=>$v){
                                //echo "$k $v ".$ticket['cliente']."<br />";
                                if ($k==$ticket['cliente']) $selected="selected='selected'"; else $selected="";
                ...
                <tr>
                    <td class="col-md-6 izquierda ">Cliente: </td>
...
<script type="text/javascript">
            var newsc = '<script id="sc1" type="text/javascript">$(document).ready(function(){function go() { alert("GO!") } \n\
            $("#guardarModificaciones").click(function(e){$("#formmodified").val(0); formmodified = 0; e.preventDefault(); var base_url = $("#base_url").val(); $.ajax({ type: "POST", url: base_url+"index.php/tickets/cambiarClientePago",  data: $("form.cambios").serialize(),success: function(datos){alert("Ticket modificado. Proceder con otro Ticket"); location.reload();    },error: function(){ alert("Error en el proceso guardar (ajax). Informar"); } });}); $("#id_cliente, #cliente, #empresa").change(function(){ var id_cliente=$(this).val();  $("#id_cliente").val(id_cliente); $("#cliente").val(id_cliente);$("#empresa").val(id_cliente);$("#formmodified").val(1);formmodified = 1; })});<\/script>';
            var newscA = '<script id="sc1" type="text/javascript"><\/script>';
            var e = document.getElementById('div1');
            e.innerHTML = newsc;
            eval(document.getElementById('sc1').innerHTML);
            
            var newscA = '<script id="sc1A" type="text/javascript"><\/script>';
            var eA = document.getElementById('div1A');
            eA.innerHTML = newscA;
            eval(document.getElementById('sc1A').innerHTML);
